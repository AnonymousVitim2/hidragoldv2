...
const vcard = 'BEGIN:VCARD\n' // JAN MUDOU
            + 'VERSION:3.0\n' // JAN MUDOU
            + 'FN: DONO ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿ⸼\n' // NOME DE CONTATO DO PROPRIETÁRIO
            + 'ORG:Criador ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿ⸼;\n' // NOME DO CRIADOR
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '[ESPERE] Estou fazendo (+2min) se não for tente novamente',
				asik: '[ESPERE] Estou fazendo (+2min) se não for tente novamente',
				success: '✔️ Sucesso ✔️',
				error: {
				...
				only: {
					...
					premium: '[❗] ESTE COMANDO É ESPECÍFICO DO USUÁRIO  *PREMIUM* se quiser virar Premium,fale com o meu proprietário wa.me/+5567996005372',
					mod: '[❗] ESTE COMANDO É ESPECÍFICO DO USUÁRIO * ModdedBot *',
					benned: 'Você não pode usar os comandos por que está banido,consequência de dar spam,entre em contato com o proprietário',
					ownerG: '[❗] Tu é o proprietário? Este é um recurso especial para o proprietário mais brabor ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿ⸼',
					ownerB: '[❗] Tu é o proprietário? Este é um recurso especial para o proprietário mais brabor ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿ⸼❌',
					userB: `──「 LISTA 」──\nOla mano !\nVocê não está registrado no banco de dados, \n\nComando : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar Hidra|16\n\n──「 ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿ⸼ 」──`,
			...
			const mod = [ownerNumber,"556796005372@s.whatsapp.net"]
			const adminbotnumber = ["556796005372@s.whatsapp.net"]
			const frendsowner = ["556796005372@s.whatsapp.net"]
			const premium = ["556796005372@s.whatsapp.net","552167446481@s.whatsapp.net","553484104285@s.whatsapp.net"]
...
const isAnime = isGroup ? anime.includes(from) : false
    ...
    if (messagesC.includes("preto")){
		...
		if (isGroupAdmins) return reply('vc é administrador, então n irei te dar ban por usar link corno')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
	...
	if (isBanned) return reply(mess.only.benned)    
		...
		if (isBanned) return reply(mess.only.benned)    
			client.updatePresence(from, Presence.composing)
			reply("bot ta aq n va se lascar")
	}
	
		if (messagesC.includes("bot")){
		if (isBanned) return reply(mess.only.benned)    
			client.updatePresence(from, Presence.composing)
			reply("krl toda hora me chama pau no seu cu")
	}
...
case 'menu11':
					...
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿ GROUP*\n\nLink : https://chat.whatsapp.com/5*', text, { quoted: mek })
					break
...
case 'brainly':
			       ...
			       case 'valxlove':
					...
					if (mentioned.length > 1) {
						...
						mentions(`╭────「 *PREMIUM👑* 」──*\n│+ *Número* : @${mentioned[0].split('@')[0]}\n│+ *Expirado*: *30 Days*\n│+ *Status*: *ATIVO*\n│ Obg por comprar o acesso Premium🥰\n*╰──────「 *ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿ* 」────`, mentioned, true)
					client.sendMessage(from, mentioned)
				    }
					break
					case 'dellvip':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿ ✨* 」\n*│+wa.me/556796005372*╰──────*「 *ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿ* 」*────*\n\n*_SE QUER SER ADMINISTRADOR  ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿ BOT_*\n*_Tipo / anúncio_*' })
					break
					case 'premiumcek':
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/556796005372*╰──────*「 * ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿ* 」*────*\n\n*_SE QUER SE TORNAR UM USUÁRIO PREMIUM  ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿ BOT_*\n*_Tipo -daftarvip*' })
					break
					case 'cekmod': 
...
client.sendMessage('./lindy/websexohjmulher.mp3', {quoted: mek, ptt:true})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					me = client.user
					uptime = process.uptime()
					teks = `*Nome Do bot* : ${me.name}\n*Outro* : * ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿ*\n*Número Bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total Block Contato* : ${blocked.length}\n*O bot está ativo a* : ${kyun(uptime)}\n*Public:* OFF`
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                 client.sendMessage(from, {displayname: "ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿ", vcard: vcard}, MessageType.contact, { quoted: mek})
                 client.sendMessage(from, 'esse é meu é o meu dono, chama ele ai po',MessageType.text, { quoted: mek} )
                 tod = await getBuffer(`https://i.ibb.co/k6qrPnW/images-1614010230280.jpg`)
            ...
            try {
                ...
                .then((response) => {
                    ...
                    for (let i = 0; i < response.data.data.length; i++) {
                        ...
                        hehex += '╚═〘 ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿ 〙'
            ...
            break
				...
				mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					...
					break
                case 'fakereplay':
                   client.fakeReply("ange mano", "melhor correr", "556796005372", MessageType.text)
                   break
				case 'infogc':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL USER BOT  ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿ* 」\n\`\`\``
					no = 0
...
case 'mining':
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*/ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿbot2412\n*', text, { quoted: mek })
					break
                case 'ttp':
                if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('556796005372@s.whatsapp.net', options, text, {quoted: mek})
...
case 'tiktok1':
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					mentions('╔══✪〘 Mencione tudo 〙✪══'+teks+'╚═〘  ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿ BOT 〙', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					reply('Pronto mestre, limpei meu chat seu lindo❤️')
					break
				case 'bc':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ * ঔৣ͜͡𝑯͢𝑰𝑫͢𝑹𝜟 𝛩𝑭 ͢𝑯𝒀͢𝑷𝑬💸⃟ꦿ Broadcast* ]\n\n${body.slice(4)}`)
						}
						reply('Transmissão de sucesso')
					}
					break
	...
	.then(() => {client.sendMessage(from, options, text)})
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *TRANSMISSÃO DO BOT* ]\n\n${body.slice(4)}`)
						}
						reply('Transmissão de sucesso ')
					}
					break
